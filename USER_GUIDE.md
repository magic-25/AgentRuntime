...
- `require_approval` 没有显式 approval provider 时默认 reject。
...
如果没有显式配置 approval provider，`require_approval` 不会被静默批准，而是返回 `approval_provider.missing`。
...
当前 contrib `ContainerSandboxBackend` 只生成和验证 container execution plan，不执行真实 Docker container。`DockerSandboxBackend` 会调用本地 Docker daemon 执行命令，但仍依赖宿主安全基线、镜像可信链、Docker daemon 权限和外部审计归档。Docker smoke evidence 只能证明当前环境可以执行 no-network/read-only/cap-drop smoke，不证明绝对逃逸防护。
...
JSONL audit sink 会使用本地 advisory file lock 串行化单节点写入，避免并发写破坏 hash chain。它不是分布式锁、外部 WORM 或合规归档。需要更强追责时，应接入宿主的 append-only sink。
...
| Container sandbox evidence | 可实际跑 | 验证本机 Docker smoke evidence；不代表 contrib backend 正在执行真实 Docker container |
...
container plan backend 是 stable candidate contract，但当前不执行真实 Docker container。Docker sandbox backend 是 preview，可真实调用本地 Docker daemon 执行 no-network/read-only/cap-drop command；Docker smoke 仍不证明绝对逃逸防护。生产隔离需要宿主侧安全基线、容器配置、网络策略、密钥隔离和审计归档共同完成。
