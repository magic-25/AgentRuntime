...
- runtime 在调用 sandbox backend 前拒绝 secret-like env，普通 subprocess 也拒绝 secret-like allowlisted env，sidecar backend 只接收 sanitized sandbox execution plan。
- execution error path 遵守 audit fail-closed。
- approval provider approve/reject/timeout、concurrent runtime audit 和 concurrent registered agent context isolation E2E。
- SQLite audit。
- tracing。
