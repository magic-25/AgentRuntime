...
2. **作为 optional integration**：GLM/Z.AI 真实 provider agent。  
   已提供 `GLM_API_KEY` 或 `ZAI_API_KEY` 环境变量路径，不提交 secret，不进默认 CI 强依赖。
...
- 待完成：使用轮换后的本地 key 跑一次真实 GLM provider 测试，并记录不含 secret 的结果摘要。

### 阶段二：真实 framework agent

- 增加 optional LangGraph graph agent test。
- graph 节点只产生一个 `echo` tool call。
- runtime 仍负责 tool execution、policy、audit。
- 该测试不进默认 core CI。
...
| `REAL_AGENT_TEST_REPORT.md` | real-agent 测试报告 |
