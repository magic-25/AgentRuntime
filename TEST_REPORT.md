...
- 当前没有验证真实 provider SDK 全量 payload，也没有验证 hosted control plane。
...
```bash
python -m pytest > .agent-runtime/test-report-2026-06-17/pytest.txt
```
...
| 范围 | 状态 | 原因 |
| --- | --- | --- |
| 真实 OpenAI / Anthropic / LangGraph / MCP / Codex provider payload 全量 replay | 未测试 | 当前只有 conformance/replay fixture，尚未收集真实 provider payload fixture |
...
### P1：真实 provider payload replay 仍缺失

adapter conformance 证明 adapter contract，但还不能证明真实 provider SDK payload 全量兼容。下一步应收集匿名真实 payload fixture，并纳入 adapter replay。
...
进入真实 design partner 前，最需要补齐的是 Code/CI runtime audit chain 和真实 provider payload replay。
