...
complete runtime report 由 `tests/test_complete_runtime_report.py` 覆盖。它运行 `examples/complete_runtime_report.py`，验证 5 个 deterministic agent scenario 都能生成完整 output：
...
测试断言 `complete-report.json` 和 `complete-report.md` 都会生成，并且每个 scenario 都包含 transcript、tool results、policy/approval/sandbox/audit governance summary、governed trace 和 audit event sequence。
